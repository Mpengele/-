void screen6(PGraphics pg) {
 ...
 if (w4.Isrunning && w4.turnStab){
  ...
  pg.text("Правое Колесо Блок 2", -width/10.06, height/4.35);
  ...
  if(w4.turnStab){
  pg.rotateZ(radians(w4.w*angleRotSim)); // this the phisical axis z
  }
  ...
  pg.endDraw();
  image(pg,(31*width)/70, (3*height)/7,(3*width)/10,(2*height)/7);
}
