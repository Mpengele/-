void screen7(PGraphics pg) {
 ...
 if (w5.Isrunning && w5.turnStab){
  ...
  pg.text("Левое Колесо Блок 3", -width/10.06, height/4.35);
  ...
  if(w5.turnStab){
  pg.rotateZ(radians(w5.w*angleRotSim)); // this the phisical axis z
  }
  ...
  pg.endDraw();
  image(pg, width/7, (5*height)/7,(3*width)/10,(2*height)/7);
}
