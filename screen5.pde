void screen5(PGraphics pg){

  //second screen for different settings
  //fill((171);
  stroke(screensLineStroke);
  strokeWeight(screensLineStrokeWeight);
  rect(width/7,(3*height)/7,(3*width)/10,(2*height)/7);
  pg.beginDraw();
  //pg.background(leftWheelgraph);
 ...
 if (Isrunning1 && turnStab1){
  ...
  pg.text("Левое Колесо Блок 2", -width/10.06, height/4.35);
  ...
  pg.endDraw();
  image(pg, width/7,(3*height)/7,(3*width)/10,(2*height)/7);
}
